...
### v0.8.0 - 指数退避重试与 Token 硬上限

**目标**: 最小粒度的生产兜底：429/500 自动退避 + 成本硬上限。

**背景**:
Agent 管控体系审计发现 Stability 稳控层完全缺失（8/16 完整，1/16 缺失）。P0 优先级 — 无限流/熔断/退避/成本上限，API 调用失控导致成本爆炸或服务不可用。本版本先实现最轻量的兜底：指数退避重试和 Token 硬上限。

**架构归属**: Runtime 层 - Stability 稳控 (P0)

**任务列表**:
- [ ] #3 指数退避重试 (`nano_agent/llm/retry.py`) — 429/500 等错误自动退避重试
- [ ] #4 Token 消耗硬上限 (`nano_agent/agent/token_budget.py` 扩展) — 单次会话 token 总量硬上限
...
**任务列表**:
- [ ] #1 LLM API 限流器 (`nano_agent/llm/rate_limiter.py`) — 调用频率限制
...
**任务列表**:
- [ ] #2 熔断器 (`nano_agent/llm/circuit_breaker.py`) — 连续失败 N 次后自动熔断
...
**任务列表**:
- [ ] #5 Prompt injection 特征过滤 (`nano_agent/agent/sanitizer.py`) — 硬性拦截 "ignore previous" / jailbreak 模板等特征模式
- [ ] #6 输入长度/格式校验 (`nano_agent/agent/sanitizer.py`) — 超长输入截断、异常格式拒绝
...
**任务列表**:
- [ ] #8 输出敏感信息拦截 (`nano_agent/agent/output_guard.py`) — 拦截 API key / 密码 / token 等泄露
- [ ] #10 中间件安全规则扩充 (`nano_agent/tools/middleware.py` 扩展) — 当前仅确认拦截，需补充自动拦截规则
...
**任务列表**:
- [ ] #9 有害内容过滤 (`nano_agent/agent/output_guard.py`) — 输出内容安全检查
...
**任务列表**:
- [ ] #13 偏差信号回流 (`nano_agent/agent/feedback.py`) — EstimationAudit 的偏差数据回流到执行策略层
- [ ] #14 自纠正循环 (`nano_agent/agent/feedback.py`) — 执行→校验→偏差→策略调整→重执行的显式闭环
...
**任务列表**:
- [ ] #18 记忆衰减策略 (`nano_agent/memory/gc.py`) — 时间衰减权重，旧记忆自动降权
- [ ] #19 记忆去重/合并 (`nano_agent/memory/gc.py`) — 相同信息多次存储时自动合并
...
**任务列表**:
- [ ] #20 长时记忆淘汰 (`nano_agent/memory/gc.py`) — 低重要性/过期记忆主动清理
...
**任务列表**:
- [ ] #21 全局状态快照 (`nano_agent/agent/snapshot.py`) — 一键保存/恢复 agent 完整状态到任意时间点
...
| v0.8.0 | 指数退避重试与 Token 硬上限 | P0 — 429/500 自动退避 + 成本硬上限 |
| v0.8.1 | LLM API 限流器 | P0 — 调用频率限制，防止突发请求打爆 API |
| v0.8.2 | 熔断器 | P0 — 连续失败后自动熔断，避免雪崩 |
...
| v0.8.13 | 长时记忆淘汰 | P2 — 低重要性/过期记忆主动清理 |
