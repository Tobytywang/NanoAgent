...
- [x] 预判机制 - 先分析问题复杂度，简单问题直接回答（已实现 → v0.7.14）
- [x] 更激进的输出精简 - 一句话回答、无表格、无列表（已实现 → v0.7.15）
...
**4. 工具返回智能摘要增强 (P1)**:
- [x] 实现 `ToolResultProcessor` 类 - 提取-摘要-结构化（已实现 → v0.7.15 `StandardToolOutput`）
- [x] 针对每个工具定制处理逻辑（已实现 → v0.7.15 工具输出标准化）
...
**任务列表**:
- [x] 实现 `run_stream_async()` 方法 - 异步生成器
- [x] LLM 流式调用 - 逐 token 接收响应
- [x] 编排层异步处理 - 异步事件处理
- [x] CLI 异步支持 - 使用 asyncio
