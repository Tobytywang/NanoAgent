...
- [ ] `ContextManager.check_and_compress()` 增加 `last_prompt_tokens` 参数
- [ ] 优先使用实际 `prompt_tokens`，fallback 到 `estimate_tokens()`
- [ ] `MessageCompressor.should_compress()` 同理增加 `last_prompt_tokens` 参数
- [ ] `react.py._think()` 中传递 `usage.prompt_tokens` 给上下文管理器
- [ ] 记录估算 vs 实际偏差日志 - 为后续校准闭环提供数据
...
**1. 统一截断比率 (P1)**:
- [ ] `react.py._observe()` 中 `max_tokens * 4` → `estimate_text_tokens()` 反算字符数
- [ ] `compressor.py` 中 `summary_max_tokens * 4` → 同上
- [ ] 消除 `result_summarizer.estimate_tokens()` 的 `len(text) // 4`，统一到 `token_utils.estimate_text_tokens()`
- [ ] 激活 `SummarizerConfig.max_summary_tokens` 死字段 - 用 `estimate_text_tokens()` 实施截断

**2. 校准闭环修复 (P1)**:
- [ ] 修正校准公式 - `avg(prompt_tokens) / estimated_prompt_tokens_for_same_messages`
- [ ] 在 `_think()` 中同时记录估算值和实际值 - 为校准提供数据
- [ ] `estimate_tokens()` 增加 `calibration_factor` 参数 - 乘以返回值
- [ ] `should_compress()` 和 `check_and_compress()` 使用校准后估算
- [ ] 校准系数值域限制 - clamp 到 [0.5, 2.0]，防止异常值
- [ ] 除零保护 + 最低采样次数 (≥3 次)
...
**1. 估算 vs 实际对比机制 (P1)**:
- [ ] 每次 LLM 调用后记录 `estimate_tokens(messages) vs usage.prompt_tokens` 偏差
- [ ] 日志/指标记录估算偏差 - 可在 `/stats` 中查看
- [ ] 基于偏差的自动校准 - 驱动校准系数更新
- [ ] 偏差过大时告警 - 估算偏差 >50% 时记录 warning

**2. 估算准确性增强 (P2)**:
- [ ] `PromptModule.token_estimate` 动态计算 - 基于 `estimate_text_tokens()` 而非硬编码
- [ ] `result_summarizer` 截断使用校准后估算
- [ ] `base_ratio` 首轮偏差修正 - 排除首轮 prompt_tokens 偏高对 base_ratio 的影响
