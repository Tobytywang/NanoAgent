...
| | 4. Sanitize 净化 | ⚠️ 部分 | 仅 prompt 级软防护 + 确认拦截 |
...
| | 10. Guard 护栏 | ⚠️ 部分 | 中间件框架存在，安全规则稀疏 |
| | 11. Validate 校验 | ⚠️ 部分 | 仅格式+置信度，无业务语义校验 |
| | 12. Feedback 反馈 | ⚠️ 部分 | 监控审计存在，反馈闭环断裂 |
| **Runtime** | 13. ToolGuard 工具防护 | ⚠️ 部分 | 无资源限制/频率限制/沙箱 |
| | 14. Stability 稳控 | ❌ 缺失 | 无限流/熔断/退避/成本上限 |
| | 15. MemoryGC 记忆迭代 | ⚠️ 部分 | 短时 FIFO 淘汰，无衰减/合并 |
| | 16. Rollback&Audit | ⚠️ 部分 | 操作级撤销，无全局快照 |

**统计**: 8/16 完整，7/16 部分，1/16 缺失
...
**当前实现**:
- `ConstraintModule` 在 system prompt 注入防护性指令（软防护）
- `ConfirmationManager` 对高风险输入要求二次确认（流程防护）

**缺失**:
- ❌ 独立的输入清洗层
- ❌ Prompt injection 特征模式硬性过滤（如 "ignore previous"、jailbreak 模板）
- ❌ PII 脱敏
- ❌ 输入长度/格式校验

**风险**: Prompt injection → agent 行为被劫持
...
**缺失**:
- ❌ 基于 schema/contract 的业务语义校验
- ❌ 结果正确性的自动化验证（如 "文件路径是否存在"、"代码是否可编译"）
- ❌ 自动修复机制
...
**缺失**:
- ❌ 工具级资源限制（CPU/内存/执行时间上限）
- ❌ 工具调用频率限制
- ❌ 工具沙箱隔离（所有工具在同一进程/权限空间）
- ❌ `dangerous` 标记仅触发确认，不阻止执行
...
**当前实现**:
- `TokenBudget` 在 token 层面做预算分配（间接关联）

**完全缺失**:
- ❌ LLM API 调用频率限制（rate limiting）
- ❌ 连续失败后的熔断机制（circuit breaker）
- ❌ 指数退避重试（exponential backoff）
- ❌ 并发请求控制
- ❌ 成本预算控制（token 消耗上限）

**风险**: API 调用失控 → 成本爆炸 / 服务不可用

**优先级**: **P0 - 生产必需**

---

#### 15. MemoryGC 记忆迭代 ⚠️ 部分
...
**缺失**:
- ❌ 显式的记忆衰减策略（时间衰减权重）
- ❌ 记忆合并/去重（同一信息多次存储产生冗余）
- ❌ 记忆质量评估和主动清理
- ❌ 长时记忆淘汰机制（只增不减，可能无限膨胀）

**风险**: 存储爆炸 / 检索效率下降

---

#### 16. Rollback & Audit 回溯兜底 ⚠️ 部分
...
**缺失**:
- ❌ 全局状态快照机制（无法一键恢复到任意时间点）
- ❌ 审计日志与回滚操作关联
- ❌ 自动回滚触发条件（如连续失败 N 次自动回滚）
...
**14. Stability 稳控** — 无限流/熔断/退避/成本上限

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 1 | LLM API 限流器 | 调用频率限制，防止突发请求打爆 API | 中 | `nano_agent/llm/rate_limiter.py` |
| 2 | 熔断器 | 连续失败 N 次后自动熔断，避免雪崩 | 中 | `nano_agent/llm/circuit_breaker.py` |
| 3 | 指数退避重试 | 429/500 等错误自动退避重试 | 低 | `nano_agent/llm/retry.py` |
| 4 | Token 消耗硬上限 | 单次会话 token 总量硬上限，防止成本失控 | 低 | `nano_agent/agent/token_budget.py` 扩展 |

---

### ⚠️ 部分实现（7 个控制点 → 19 项）

**4. Sanitize 净化** — 仅 prompt 级软防护

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 5 | Prompt injection 特征过滤 | 硬性拦截 "ignore previous" / jailbreak 模板等特征模式 | 中 | `nano_agent/agent/sanitizer.py` |
| 6 | 输入长度/格式校验 | 超长输入截断、异常格式拒绝 | 低 | `nano_agent/agent/sanitizer.py` |
| 7 | PII 脱敏 | 手机号/身份证/API key 等敏感信息自动脱敏（可选） | 高 | `nano_agent/agent/sanitizer.py` |

**10. Guard 护栏** — 中间件框架存在，安全规则稀疏

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 8 | 输出敏感信息拦截 | 拦截 API key / 密码 / token 等泄露 | 中 | `nano_agent/agent/output_guard.py` |
| 9 | 有害内容过滤 | 输出内容安全检查（可选，视场景而定） | 高 | `nano_agent/agent/output_guard.py` |
| 10 | 中间件安全规则扩充 | 当前仅确认拦截，需补充自动拦截规则 | 中 | `nano_agent/tools/middleware.py` 扩展 |

**11. Validate 校验** — 仅格式+置信度，无业务语义校验

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 11 | 结果正确性验证 hook | 如文件路径是否存在、代码是否可编译、命令是否真正执行成功 | 高 | `nano_agent/agent/validator.py` |
| 12 | Schema-based 校验 | 工具返回值按预期 schema 校验，不符则标记/修复 | 中 | `nano_agent/agent/validator.py` |

**12. Feedback 反馈** — 监控审计存在，闭环断裂

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 13 | 偏差信号回流 | EstimationAudit 的偏差数据回流到执行策略层 | 高 | `nano_agent/agent/feedback.py` |
| 14 | 自纠正循环 | 执行→校验→偏差→策略调整→重执行的显式闭环 | 高 | `nano_agent/agent/feedback.py` |

**13. ToolGuard 工具防护** — 无资源限制

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 15 | 工具执行超时 | 单次工具调用超时上限，防止挂死 | 低 | `nano_agent/tools/resource_limiter.py` |
| 16 | 工具调用频率限制 | 单工具/全局调用频率上限 | 低 | `nano_agent/tools/resource_limiter.py` |
| 17 | 工具沙箱隔离 | 进程级隔离（实现成本高，可选） | 高 | `nano_agent/tools/sandbox.py` |

**15. MemoryGC 记忆迭代** — 短时 FIFO 淘汰，长时只增不减

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 18 | 记忆衰减策略 | 时间衰减权重，旧记忆自动降权 | 中 | `nano_agent/memory/gc.py` |
| 19 | 记忆去重/合并 | 相同信息多次存储时自动合并 | 中 | `nano_agent/memory/gc.py` |
| 20 | 长时记忆淘汰 | 低重要性/过期记忆主动清理，防止无限膨胀 | 中 | `nano_agent/memory/gc.py` |

**16. Rollback & Audit** — 操作级撤销，无全局快照

| # | 工作项 | 说明 | 复杂度 | 建议位置 |
|---|--------|------|--------|----------|
| 21 | 全局状态快照 | 一键保存/恢复 agent 完整状态到任意时间点 | 高 | `nano_agent/agent/snapshot.py` |
| 22 | 审计-回滚关联 | 从审计日志直接触发回滚操作 | 中 | `nano_agent/agent/snapshot.py` |
| 23 | 条件触发自动回滚 | 连续失败 N 次自动回滚到上一检查点 | 中 | `nano_agent/agent/snapshot.py` |

---

### 工作项统计

| 优先级 | 工作项编号 | 数量 | 核心必做 | 可选 |
|--------|-----------|------|---------|------|
| P0 生产必需 | #1-4 | 4 | 4 | 0 |
| P1 安全必需 | #5-10 | 6 | 5 | 1 (#9) |
| P2 鲁棒性 | #11-20 | 10 | 8 | 2 (#7, #17) |
| P3 完备性 | #21-23 | 3 | 3 | 0 |
| **合计** | | **23** | **20** | **3** |
