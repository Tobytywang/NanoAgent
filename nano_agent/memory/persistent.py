...
class PersistentMemory(BaseMemory):
    ...
    def _load_or_init(self) -> None:
        ...
        if self.storage.session_exists(self.session_id):
            ...
            # Strip stale system prompts and [System]-prefixed messages from stored sessions.
            # System prompts are regenerated by _setup_system_prompt() on every startup,
            # so persisted copies are redundant and can go stale (e.g. empty tools list).
            filtered = [
                e
                for e in entries
                if e.role != "system"
                and not (e.role == "user" and e.content.startswith("[System]"))
            ]
