...
class TokenAnalyzer:
    ...
    def analyze_llm_call(
        ...
        for msg in input_messages:
            ...
            if role == "system":
                ...
                self._record_tool_output(tool_name, msg_tokens)
        ...
        if tool_calls:
            for tc in tool_calls:
                ...
                # 估算工具调用参数 Token
                args_tokens = self._estimate_dict_tokens(tc.get("arguments", {}))
                self._record_tool_input(tool_name, args_tokens)
        ...
        if isinstance(content, str):
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    total += len(item["text"]) // 3 + 1
            return total
        return 0

    def _estimate_dict_tokens(self, d: dict) -> int:
        """
        估算字典内容的 Token 数。
    ...
    def _record_tool_output(self, tool_name: str, tokens: int) -> None:
        """
        记录工具输出 Token。
        ...
        self._tool_token_usage[tool_name].output_tokens += tokens

    def _record_tool_input(self, tool_name: str, tokens: int) -> None:
        """
        记录工具调用参数 Token。
        ...
        self._tool_token_usage[tool_name].input_tokens += tokens
